...
{
    public partial class Hearing_Notice : System.Web.UI.Page
    {
...

            {
                ...
                if (Request.QueryString["Flag"] != null)
                {
                    ...
                    if (Request.QueryString["Flag"].ToString() == "1" || Request.QueryString["Flag"].ToString() == "2")// Success eSign
                    {
                        ...
                        {
                            if (Request.QueryString["Response_From"].ToString() == "Hearing_Notice")
                            {
                                DataTable dt = clsNoticeBAL.InserteSignDSC_Status_OrdersheetNextNotice(Convert.ToInt32(Session["AppID"].ToString()), "1", "", Convert.ToInt32(Session["Hearing_ID_Previous"].ToString()));
                        ...
                        {
                            if (Request.QueryString["Response_type"].ToString() == "Next_Notice")
                            {
                                DataTable dt = clsNoticeBAL.InserteSignDSC_Status_NextNotice(Convert.ToInt32(Session["AppID"].ToString()), "1", "", GetLocalIPAddress(), Convert.ToInt32(Session["Notice_ID"].ToString()));
                                pnlEsignDSC.Visible = false;
                                pnlSend.Visible = true;
                            }
                        }
                    }
                    else if (Request.QueryString["Flag"].ToString() == "0" && Request.QueryString["Response_type"].ToString() == "Next_Notice") //faild Notice eSign 
                    {
                        Flag = 3;
                ...
                if (Session["AppID"] != null)
                {
                    ...
                    if (dt.Rows[0]["STATUS_ID"].ToString() == "38")
                    {
                ...
                if (Flag == 1)
                {
                    ...
                    if (Session["CaseNum"] != null)
                    {
                        ...
                        dsDocRecent = clsNoticeBAL.GetHearingPath(ViewState["Case_Number"].ToString(), appid);
                        if (dsDocRecent != null)
                        {
                            if (dsDocRecent.Tables.Count > 0)
                            {
                                ...
                                {
                                    string fileName = dsDocRecent.Tables[0].Rows[0]["SIGNED_DocPath"].ToString();
                ...
                if (dtt.Rows[0]["STATUS_ID"].ToString() == "38")
                {
                ...
                if (dtt.Rows[0]["STATUS_ID"].ToString() == "39" || dtt.Rows[0]["STATUS_ID"].ToString() == "40")
                {
                ...
                if (dtt.Rows[0]["STATUS_ID"].ToString() == "63")
                {
        ...
        {
            string serverpath = Server.MapPath("~/CoS_NextNoticeAllSheetDoc/" + filename);
            if (!File.Exists(serverpath))
            {
                ConvertHTMToPDF(filename, "~/CoS_NextNoticeAllSheetDoc/", "<p>Order Sheet</p>");
            }
            else
            {
            ...
            ViewState["ALLDocCAddedPDFPath"] = "~/CoS_NextNoticeAllSheetDoc/" + filename;
            ViewState["CoS_NextNoticeAllSheetDoc"] = serverpath;
        }
        ...
        public void CraetSourceFile(int APP_ID)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    string sourceFile = ViewState["CoS_NextNoticeAllSheetDoc"].ToString();
        ...
        private void setAllPdfPath(string vallPdfPath)
        {
            ...
            //{
            //    ifPDFViewerAll.Src = "~/CoS_NextNoticeAllSheetDoc/" + All_OrderSheetFileNme;
            //}
            if (File.Exists(Server.MapPath(vallPdfPath)))
            {
                ifPDFViewerAll.Src = "~/CoS_NextNoticeAllSheetDoc/" + All_OrderSheetFileNme;
        ...
        private void SaveNoticePDF()
        {
            try
            {
                ...
                stringBuilder.Append("</div>");
                stringBuilder.Append("<br/>");
                stringBuilder.Append("<b style='float: right; text-align: center; padding: 2px 0 5px 0; position: relative;top: 220px;left:1px; color:#fff;'>#8M2h8A4@N78O%bJd<br/><br/> </b>");
                stringBuilder.Append("<b style='float: right; text-align: center; padding: 2px 0 5px 0; position: relative;top: 240px;left:150px;'>स्थान- जिला पंजीयक कार्यालय, " + lblDRoffice.Text + "<br/>जारी दिनांक: " + lblTodate.Text + " <br/> <br/></b> ");

                stringBuilder.Append("<b style='float: left; text-align: left; padding: 2px 0 5px 5px; position: relative;top: 10px;'>नोट : पार्टी दस्तावेज पंजीयन के दौरान संपदा में पंजीकृत मोबाइल नंबर के साथ उपस्थित हो ।<br/></b> ");
        ...
        protected void btnSendNotice_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //return;
                //int Flag = 1;
                Response.Redirect("Cases_In_Hearing.aspx");
            }
            catch (Exception ex)
            {
        ...
        protected void BtnSaveDraft_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //DataTable dtStatus = clsNoticeBAL.Get_Status_Notice(Convert.ToInt32(ViewState["AppID"].ToString()), Convert.ToInt32(ViewState["NoticeIID"].ToString()));
                DataTable dtStatus = clsNoticeBAL.Get_Status_NextNotice(Convert.ToInt32(ViewState["AppID"].ToString()), Convert.ToInt32(ViewState["HearingId"].ToString()));

                if (dtStatus.Rows.Count > 0)
                {
                    if (dtStatus.Rows[0]["Notice_Id"].ToString() != "" && dtStatus.Rows[0]["Notice_Id"].ToString() != "0")
                    ...
                    if (File.Exists(FileNamefmFolder))
                    {
                        if (ddl_SignOption.SelectedValue == "1")
                        {
                            ...
                            if (File.Exists(FileNamefmFolder))
                            {
                                ...
                                //ResponseURL = Request.Url.OriginalString.Replace(Path.GetFileName(Request.Url.AbsoluteUri), "ResponseFromeSign_Hearing_Notice.aspx?Case_Number=" + ViewState["Case_Number"] + "&App_Id=" + Session["AppID"].ToString() + "&AppNo=" + Session["Appno"] + "&HearingDate=" + ViewState["HearingDate"] + "&Party_ID=" + Session["Party_ID"] + "&Notice_ID=" + Session["Notice_ID"].ToString() + "&Response_type=Hearing_Notice");
                                ResponseURL = Request.Url.OriginalString.Replace(Path.GetFileName(Request.Url.AbsoluteUri), "ResponseFromeSign_NextNotice.aspx?Response_type=Next_Notice");

                                //getdata();
                                //DataTable dt = clsNoticeBAL.InserteSignDSC_Status_NextNotice(App_ID, ddl_SignOption.SelectedValue, "", GetLocalIPAddress(), Notice_id);
        ...
        protected void btnFinalSubmit_Click(object sender, EventArgs e)
        {
            ...
            SaveNoticePDF();
            DataTable dtUp = clsNoticeBAL.ChangeFinalSubmit_NextNotice(Convert.ToInt32(ViewState["AppID"].ToString()));
            pnlEsignDSC.Visible = true;
            pnlSaveDraft.Visible = false;
