...
namespace CMS_Sampada.CoS
{
    ...
    public partial class PendingForNoticeDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Flag == 1)
            {
...

                {
                    ...
                    if (Request.QueryString["Case_Number"] != null)
                    {
                        ...
                        string hearing_id = Request.QueryString["hearing_id"].ToString();

                        if (Request.QueryString["hearing_id"] != null)
                        {
                            hearing_id = Request.QueryString["hearing_id"].ToString();
                            ViewState["hearing_id"] = Request.QueryString["hearing_id"].ToString();
                            DataSet dsGetNotice_Id = new DataSet();
                            dsGetNotice_Id = clsNoticeBAL.Get_NoticeDetails_ForPendingDetails(3,Convert.ToInt32( appid), "", 0, Convert.ToInt32 (hearing_id));
                            if (dsGetNotice_Id.Tables[0].Rows.Count > 0)
                            {
                               DataSet dsGetNotice = clsNoticeBAL.Get_NoticeDetails_ForPendingDetails(2, Convert.ToInt32(appid), "", Convert.ToInt32(dsGetNotice_Id.Tables[0].Rows[0]["Notice_ID"].ToString()), Convert.ToInt32(hearing_id));

                                ViewState["Case_Number"] = Request.QueryString["Case_Number"].ToString();
                                string appid_ = Request.QueryString["App_Id"].ToString();
                                string numbs_ = Request.QueryString["Case_Number"].ToString();
                                string Notice_ID = dsGetNotice_Id.Tables[0].Rows[0]["Notice_ID"].ToString();
                                Session["Notice_ID"] = "";
                                Session["Party_ID"] = "";
                                ...
                                Session["Party_ID"] = dsGetNotice.Tables[0].Rows[0]["party_id"].ToString();
                                string Party = dsGetNotice.Tables[0].Rows[0]["party_id"].ToString();
                                ...
                                Session["HearingDate"] = dsGetNotice.Tables[0].Rows[0]["Hearing_Date"].ToString();
                                //int appid = 2;

                                ViewState["AppID"] = appid;
                                Session["AppID"] = appid;
                                //string da = Session["Party_ID"].ToString();
                                string Appno_ = Request.QueryString["AppNo"].ToString();
                                List<string> uniques = Appno_.Split(',').Reverse().Distinct().Take(2).Reverse().ToList();
                                string newStr = string.Join(",", uniques);
                                Appno_ = newStr;
                                Appno_ = Appno_.Replace(",", "");
                                //string Appno = "IGRSCMS1000102";
                                ViewState["Appno"] = Appno_;
                                Session["Appno"] = Appno_;
                                edit_notice.Visible = false;
                                DateTime Todate_ = DateTime.Now;
                                lblTodate.Text = DateTime.Now.ToString("dd/MM/yyyy");
                                string HearingDT_ = DateTime.Now.ToString(Session["HearingDate"].ToString());
                                ...
                                GrdAddCopy_Details.DataSource = dataTable;
                        ...
                        GetPartyDetail();
                        GetPreviousProcedding();
                        int HearingId = Convert.ToInt32(ViewState["hearing_id"]);
            ...
            else
            {
                ...
                if (Request.QueryString["Case_Number"] != null)
                {
                    ViewState["Case_Number"] = Request.QueryString["Case_Number"].ToString();
                    string appid = Request.QueryString["App_Id"].ToString();
                    string numbs = Request.QueryString["Case_Number"].ToString();
                    ...
                    Session["Appno"] = "";
                    Session["Notice_ID"] = Notice_ID;
                    Session["Party_ID"] = Request.QueryString["Party_ID"].ToString();
                    string Party= Request.QueryString["Party_ID"].ToString();
                    //List<string> uniquesParty = Party.Split(',').Reverse().Distinct().Take(2).Reverse().ToList();
                    //string newStrparty = string.Join(",", uniquesParty);
                    Session["Party_ID"] = Party;
                    Session["HearingDate"] = Request.QueryString["HearingDate"].ToString();
                    //int appid = 2;
        ...
        private void GetPartyDetail()
        {
            ...
            if (ViewState["Case_Number"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    lblDepartment.Text = dt.Rows[0]["DEPARTMENT_NAME_EN"].ToString();
            ...
            foreach (GridViewRow gvrow in grdCaseList.Rows)
            {
                ...
                if (chk != null & chk.Checked)
                {
                    ...
                    if (DSPartyDisplay.Tables.Count > 0)
                    {
                        ...
                        FatherName = DSPartyDisplay.Tables[0].Rows[0]["PARTYFATHER_ORHUSBAND_ORGUARDIANNAME"].ToString();
                        Address = DSPartyDisplay.Tables[0].Rows[0]["PARTY_ADDRESS"].ToString();
        ...
        private void SaveNotice(string Path)
        {
            try
            {
                ...
                DataTable dtUp = clsNoticeBAL.InsertIntoNotice(Convert.ToInt32(ViewState["AppID"].ToString()), 0, ViewState["Case_Number"].ToString(), ViewState["ActualPath"].ToString(), summernote.Value, "", "", "", V_HEARINGDATE, 0, 0, "", 0,0, txtCopyName.Text, txtCopyEmail.Text, txtMobile.Text, txt.Text, txtWhatsApp.Text, Convert.ToInt32(ViewState["HearingId"].ToString()));
        ...
        private void SaveNoticePDF()
        {
            try
            {
                ...
                //stringBuilder.Append("<div class='main-box html_box htmldoc'>");
                //stringBuilder.Append("<div class='main - box htmldoc' style='width: 100 %; margin: 0 auto; text - align: center; border: 1px solid #ccc; padding: 30px 30px 30px 30px;'>");
                //stringBuilder.Append("<h2 class='h - 2'>कार्यालय जिला पंजीयक एवं न्यायालय कलेक्टर ऑफ स्टाम्प जिला भोपाल (म.प्र.)</h2>");
                //stringBuilder.Append("<h3 class='h - 3'>आई.एस.बी.टी. परिसर, मेजनाईन फ्लोर, हबीबगंज भोपाल < br >ई - मेल - igrs@igrs.gov.in</h3> ");
                //stringBuilder.Append("<h2 class='h - 22'><b>अधिनियम 1899 की धारा 33 के स्टाम्प प्रकरणों की सुनवाई हेतु सूचना पत्र< br >प्रकरण क्रमांक -" + lblCaseNo.Text + "धारा - 33 </ b ></ h2 > ");
                //stringBuilder.Append("<h2 class='h - 22'>मध्यप्रदेश शासन</h2>");
                //stringBuilder.Append("<h2 class='h - 22'>विरुद्ध</h2>");
                //stringBuilder.Append("<br>");
                //stringBuilder.Append("<h3 class='h - 3 text - left'> " + lblDepartment.Text + " <br ><br> <b> आवेदक(प्रथम पक्षकार) </ b >< br >< br >" + lblRecord.Text + "<br><br> <b>अनावेदक(द्वितीय पक्षकार) </b></h3>");
                //stringBuilder.Append("<br>");
                //stringBuilder.Append("<div class='devidetwo'>");
                //stringBuilder.Append("<div>");
                ////stringBuilder.Append(summernote.Value);
                //stringBuilder.Append(divCon);
                //stringBuilder.Append("</div>");
                //stringBuilder.Append("</div>");
                //stringBuilder.Append("<b class='stamp'>स्थान- जिला पंजीयक कार्यालय, भोपाल-2 <br/>जारी दिनांक:" + lblTodate.Text + " <br/> <br/></b> ");
                //stringBuilder.Append("<div>");
                //stringBuilder.Append("</div>");
                //stringBuilder.Append("<br/>");
                //stringBuilder.Append("<br/>");
                ...
                //stringBuilder.Append("<table width='100%' cellspacing='0' cellpadding='5' border='1' border-spacing='0' style='margin: 0; padding: 0; '><tr><th style='text-align: left;'>क्र.सं.</th><th style='text-align: left;'></th></tr>");
                //int srno1 = 1;
                //for (int i = 0; i < ((DataTable)ViewState["CopyDeatils"]).Rows.Count; i++)
                //{
                //    stringBuilder.Append("<tr><td><b>" + srno1 + "<b></td><td>" + ((DataTable)ViewState["CopyDeatils"]).Rows[i]["CopyContent"] + "</td><td>");
                //    srno1++;
                //}
                ...
                stringBuilder.Append("<div class='main-box htmldoc' style='height:1300px;margin: 0 auto;  border: 1px solid #ccc; padding: 30px 30px 30px 30px;'>");

                stringBuilder.Append("<h2 style='font-size: 20px; margin: 0; font-weight: 600; text-align: center '>कार्यालय जिला पंजीयक एवं न्यायालय कलेक्टर ऑफ स्टाम्प जिला भोपाल (म.प्र.)</h2>");
                stringBuilder.Append("<h3 style='margin: 0; margin: 10px; font-size: 20px; text-align: center '>आई.एस.बी.टी. परिसर, मेजनाईन फ्लोर, हबीबगंज भोपाल <br> ई - मेल - igrs@igrs.gov.in</h3> ");
                stringBuilder.Append("<h2 style='margin: 0; margin: 10px; font-size: 20px; text-align: center '><b>अधिनियम 1899 की धारा 33 के स्टाम्प प्रकरणों की सुनवाई हेतु सूचना पत्र <br> प्रकरण क्रमांक -" + lblCaseNo.Text + " धारा - 33 </b></h2> ");
                stringBuilder.Append("<h2 style='margin: 0; margin: 10px; font-size: 20px; text-align: center '>मध्यप्रदेश शासन</h2>");
                stringBuilder.Append("<h2 style='margin: 0; margin: 10px; font-size: 20px; text-align: center '>विरुद्ध</h2>");
                stringBuilder.Append("<br>");
                stringBuilder.Append("<div>");
                ...
                stringBuilder.Append("</div>");
                stringBuilder.Append("<br/>");
                stringBuilder.Append("<b style='float: right; text-align: center; padding: 2px 0 5px 0; position: relative;top: 100px;left:1px; color:#fff;'>#8M2h8A4@N78O%bJd<br/><br/> </b>");
                stringBuilder.Append("<b style='float: right; text-align: center; padding: 2px 0 5px 0; position: relative;top: 150px;left:150px;'>स्थान- जिला पंजीयक कार्यालय, भोपाल-2,<br/>जारी दिनांक: " + lblTodate.Text + " <br/> <br/></b> ");
        ...
        protected void btnSendNotice_Click(object sender, EventArgs e)
        {
            ...
            //return;
            int Flag = 1;
            Response.Redirect("Notice_Proceeding.aspx?Case_Number=" + ViewState["Case_Number"] + "&App_Id=" + Session["AppID"].ToString() + "&AppNo=" + Session["Appno"].ToString() + "&Notice_ID=" + Session["Notice_ID"].ToString() + "&Flag=" + Flag);
        ...
        public void sendwhatsapp1(string Whatsapp, string Name, string CaseNo, string RegistrtionNo, string noticepdf, string PartyID, string Notice_ID)
        {
            ...
            using (WebClient webClient = new WebClient())
            {
                ...
                noticepdf = "/SampadaCMS" + noticepdf;
                string noticepdfsave = noticepdf;
                string Link = "https://" + authority + noticepdf;
                string msgurl = authority + noticepdf;
